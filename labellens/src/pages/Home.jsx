...
export default function Home() {
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(err.detail || 'Scan failed. Please try again.');
      }
  ...
  return (
    <div style={{ flex: 1, paddingTop: '64px', display: 'flex', flexDirection: 'column', position: 'relative' }}>
      {/* Background grid */}
      <div style={{
        ...
        <AnimatePresence mode="wait">
          {loading ? (
            <motion.div key="loader" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ width: '100%', maxWidth: '560px' }}>
              <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-xl)', padding: '1rem' }}>
                <Loader />
              </div>
            </motion.div>
          ) : (
            ...
            >
              ...
              <div style={{ textAlign: 'center', maxWidth: '540px' }}>
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
                  style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: 'var(--accent-dim)', border: '1px solid rgba(168,255,62,0.25)', borderRadius: '999px', padding: '5px 14px', marginBottom: '1.2rem' }}
                >
                ...
                <motion.h1 initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
                  style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'clamp(2rem, 5vw, 3rem)', letterSpacing: '-0.04em', lineHeight: 1.1, marginBottom: '1rem', color: 'var(--text-primary)' }}
                >
                  Know what's{' '}
                  <span style={{ color: 'var(--accent)', textShadow: '0 0 40px rgba(168,255,62,0.4)' }}>really</span>{' '}
                  in your food.
                </motion.h1>

                <motion.p initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
                  style={{ color: 'var(--text-secondary)', fontSize: '1rem', lineHeight: 1.7, maxWidth: '420px', margin: '0 auto' }}
                >
              ...
              {/* Upload */}
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} style={{ width: '100%' }}>
              ...
              {error && (
                <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
                  style={{ background: 'var(--red-dim)', border: '1px solid rgba(255,78,78,0.3)', borderRadius: 'var(--radius-md)', padding: '12px 18px', color: 'var(--red)', fontSize: '13px', fontFamily: 'var(--font-display)', maxWidth: '480px', width: '100%', textAlign: 'center' }}
                >
                  ⚠️ {error}
                </motion.div>
              )}
              ...
              <motion.button
                ...
                disabled={!file}
                style={{
                  padding: '16px 48px', borderRadius: 'var(--radius-md)',
                  ...
                  letterSpacing: '0.02em', cursor: file ? 'pointer' : 'not-allowed',
                  transition: 'all 0.25s', display: 'flex', alignItems: 'center', gap: '10px',
                }}
              >
                <span>🔍</span><span>Scan Ingredients</span>
              </motion.button>
              ...
              >
                {['🔒 Privacy First', '⚡ BERT Model', '🧪 Ingredient Verified'].map(badge => (
                  <span key={badge} style={{ fontSize: '12px', color: 'var(--text-muted)', fontFamily: 'var(--font-display)' }}>{badge}</span>
                ))}
              </motion.div>
            </motion.div>
          )}
